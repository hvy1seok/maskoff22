...
if [ -f "${SCRIPT_DIR}/epoch=13-step=364.ckpt" ]; then
    chmod o+r "${SCRIPT_DIR}/epoch=13-step-364.ckpt"
fi
...
docker run --rm \
    ...
    --volume "${SCRIPT_DIR}/epoch=13-step-364.ckpt":/opt/ml/model/epoch=13-step-364.ckpt:ro \
    "$DOCKER_IMAGE_TAG"
